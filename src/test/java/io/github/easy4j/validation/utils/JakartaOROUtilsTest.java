...
class JakartaOROUtilsTest {
    ...
    void shouldSupportPatternMatcherInputOverloads() {
        ...
        MatchResult result = JakartaOROUtils.getMatchResult(
                "\\d+", Perl5Compiler.DEFAULT_MASK, resultInput);
        assertNotNull(result);
        assertEquals("34", result.group(0));
        assertNotNull(JakartaOROUtils.replaces(
                "\\d+", Perl5Compiler.DEFAULT_MASK, replaceInput));
    }

    @Test
    void shouldHandleBlankAndMalformedPatternMatcherInputOverloads() {
        assertNull(JakartaOROUtils.getMatchResult(
                "", Perl5Compiler.DEFAULT_MASK, new PatternMatcherInput("value")));
        assertEquals("value", JakartaOROUtils.replaces(
                "", Perl5Compiler.DEFAULT_MASK, new PatternMatcherInput("value")));
        assertFalse(JakartaOROUtils.matches(
                "[invalid", Perl5Compiler.DEFAULT_MASK, new PatternMatcherInput("value")));
        assertFalse(JakartaOROUtils.matchesPrefix(
                "[invalid", Perl5Compiler.DEFAULT_MASK, new PatternMatcherInput("value")));
        assertFalse(JakartaOROUtils.contains(
                "[invalid", Perl5Compiler.DEFAULT_MASK, new PatternMatcherInput("value")));
        assertNull(JakartaOROUtils.getMatchResult(
                "[invalid", Perl5Compiler.DEFAULT_MASK, new PatternMatcherInput("value")));
        assertEquals("value", JakartaOROUtils.replaces(
